...
const RepoCard = ({ icon, status, name, description, branch, lastActivity }: RepoCardProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <Icon name={icon} className="repo-icon" sx={{ color: 'primary.main' }} />
        <Box
          sx={{
      ...
      <Typography variant="body1" sx={{ fontWeight: 700, color: 'onSurface', mb: 0.5 }}>
        {name}
      </Typography>
      <Typography variant="caption" sx={{ color: 'onSurfaceVariant', display: 'block', mb: 2 }}>
        {description}
      </Typography>
      <Box
        sx={{
